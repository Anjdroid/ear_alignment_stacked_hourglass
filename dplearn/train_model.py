...
class TrainModel:
    ...
    def calc_loss(self, lds, output_pts, loss_fn, img):
        ...
        for i,ld in enumerate(lds):
            ...
            for j in range(self.num_lds):
                ht = sample['landmarks'][j]
                #ht = sample['landmarks']
                #print(ht.shape)
                #print(output_pts.shape)
                ht = cv.resize(ht, (64, 64))
                ht = torch.from_numpy(ht).to(self.device)               
                ht = ht.type(torch.cuda.FloatTensor)

                #print(ht.shape)
                #print(output_pts[i][j].shape)

                loss2 = loss_fn(output_pts[i][j], ht)                    
                loss = loss + loss2
    ...
    def train_1(self, train_loader, model, loss_fn, optimizer, epoch, best_loss, epoch_since_improv, loss_list, iter_list):
        ...
        for batch_i, data in enumerate(train_loader):
            ...
            loss = self.calc_loss(lds, output_pts, loss_fn, images)
            step_loss = loss.item()
    ...
    def train_model(self):
        ...
        # adam optimizer
        optimizer = self.setup_optimizer(model, 3e-8, 1e-5, 0)
        ...
        for epoch in range(epoch_s, num_epochs, 1):
            ...
            loss, epoch_since_improv, step_loss = self.train_1(train_loader, model, loss_fn, optimizer, epoch, step_loss, epoch_since_improv, loss_list, iter_list)
